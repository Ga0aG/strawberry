...
mtc::Task MTCTaskNode::createTask() {
  ...
  // current_state_ptr, which is now used to forward information about the
  // object pose and shape to the inverse kinematics solver.
  ...
  { // grasp
    ...
    // declare the task properties from the parent task in the new serial
    // container, and use configureInitFrom() to initialize them. This allows
    // the contained stages to access these properties.
    task.properties().exposeTo(grasp->properties(),
                               {"eef", "group", "ik_frame"});
    grasp->properties().configureInitFrom(mtc::Stage::PARENT,
                                          {"eef", "group", "ik_frame"});

    { // grasp - approach_object
    ...
    { // grasp - sample grasp pose (Generator)
      // Sample grasp pose
      ...
      // This is used to determine the number of poses to generate; when
      // generating solutions, MoveIt Task Constructor will try to grasp the
      // object from many different orientations, with the difference between
      // the orientations specified by the angle delta.
      stage->setAngleDelta(M_PI / 12);
      // This is used to forward information about the object pose and shape to
    ...
    { // grasp - allow collision (hand,object)
  ...
  ---- *               Generate Place Pose                *
     ***************************************************/
    {
      ...
      wrapper->setMinSolutionDistance(1.0);
      wrapper->setIKFrame(hand_frame); //"object");
      wrapper->properties().configureInitFrom(mtc::Stage::PARENT,
                                              {"eef", "group"});
      wrapper->properties().configureInitFrom(mtc::Stage::INTERFACE,
                                              {"target_pose"});
