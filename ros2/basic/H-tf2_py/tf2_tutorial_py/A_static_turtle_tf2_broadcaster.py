...
def quaternion_from_euler(ai, aj, ak): # degree
    ai = math.radians(ai)/2.0
    aj = math.radians(aj)/2.0
    ak = math.radians(ak)/2.0
    ci = math.cos(ai)
    si = math.sin(ai)
    cj = math.cos(aj)
    sj = math.sin(aj)
...
def main():
    ...
    if len(sys.argv) != 8:
        logger.info('Invalid number of parameters. Usage: \n'
                    ...
                    'child_frame_name x y z roll pitch yaw (fixed frame)')
        sys.exit(1)
